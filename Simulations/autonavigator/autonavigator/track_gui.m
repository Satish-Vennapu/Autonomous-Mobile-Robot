function dist = track_gui(x,y,angle_v,angle_t,conf,v,status_cb,tft,trt,tvb,delta,and_method,traj_algo)
...
while(1)
  ...
  %  if (check_boundary(x0,y0)) break; end;
  %  if (check_boundary(x1,y1)) break; end;
  %  if (check_boundary(x2,y2)) break; end;
  %  if (check_boundary(x3,y3)) break; end;
    ...
    if v<0
        ...
        set(l4,'linewidth',3,'color',[c1 c2 c3]);
    ...
    switch traj_algo
        ...
        case 2
            ...
            angle_vn=angle_v+steer_v;
            gamma_=angle_tn+angle_vn;
            var_B=d*cosd(angle_v)+v*cosd(gamma_)-d*cosd(angle_vn);
            var_C=cosd(angle_vn);
            v_rear=var_B/var_C;
            %new position of rear axle mid point
    ...
    a=sprintf('\n');
    fwrite(fid,a);
    qp=strcat('X_POS : ',num2str(x),' Y_POS : ',num2str(y),' ANGLE_V : ',num2str(angle_v),' ANGLE_T : ',num2str(angle_t),' V_REAR : ',num2str(v_rear),' V_FRONT : ',num2str(v),' STEER_V : ',num2str(steer_v),' STEER_T : ',num2str(steer_t),' ORI_STEER : ',num2str(ori_steer));
    fwrite(fid,qp);
    angle_fi=angle_t+angle_v;
